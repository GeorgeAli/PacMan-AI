...
class SearchAgent(Agent):
    ...
    def __init__(
        ...
        if "heuristic" not in func.__code__.co_varnames:
            ...
            print(
                ("[SearchAgent] using function %s and heuristic %s" % (fn, heuristic))
            )
...
def foodHeuristic(state, problem):
    ...
    for item in foodGrid.asList():
        ...
        current_path.append(mazeDistance(position,item,problem.startingGameState))

        if problem.heuristicInfo['wallCount']:
            break

    return max(current_path)
