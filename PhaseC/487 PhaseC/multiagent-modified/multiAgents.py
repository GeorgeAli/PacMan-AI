...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def max_value(gameState, depth):
            ...
            legal_actions = gameState.getLegalActions(0)
            if legal_actions:
                legal_actions.pop(len(legal_actions) - 1)
            value_to_compare = -float("inf")
            best_action = -1

            if len(legal_actions) == 0 or depth == self.depth:
                return better(gameState), -1

            for every_action in legal_actions:
                move = probable_action(
                    gameState.generateSuccessor(0, every_action), depth
                )
                if value_to_compare <= move[0]:
                    value_to_compare, best_action = move[0], every_action
            return value_to_compare, best_action

        def probable_action(gameState, depth):
            ...
            for every_action in legal_actions:
                sucsValue = max_value(
                    gameState.generateSuccessor(0, every_action), depth + 1
                )
                prob = float(sucsValue[0]) / lenth_of_la
...
def betterEvaluationFunction(currentGameState):
    ...
    score -= 200 * len(currentGameState.getCapsules())
    ...
    for every_ghost in range(len(ghost)):
        if ghost[every_ghost].scaredTimer:
            ...
            ghost_distances.append(
                manhattanDistance(pos, ghost[every_ghost].getPosition())
            )
